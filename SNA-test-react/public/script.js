...
// Custom Confirm Dialog - replaces native confirm() with styled modal
let _confirmDialogActive = false;
function showConfirmDialog(message) {
  if (_confirmDialogActive) return Promise.resolve(false);
  _confirmDialogActive = true;

  return new Promise((resolve) => {
    let overlay = document.getElementById("sna-confirm-overlay");
    if (!overlay) {
      overlay = document.createElement("div");
      overlay.id = "sna-confirm-overlay";
      overlay.className = "sna-confirm-overlay";
      overlay.innerHTML = `
        <div class="sna-confirm-dialog">
          <p class="sna-confirm-message"></p>
          <div class="sna-confirm-buttons">
            <button type="button" class="sna-confirm-btn sna-confirm-cancel">Cancel</button>
            <button type="button" class="sna-confirm-btn sna-confirm-ok">OK</button>
          </div>
        </div>
      `;
    ...
    const cancelBtn = overlay.querySelector(".sna-confirm-cancel");
    const okBtn = overlay.querySelector(".sna-confirm-ok");

    function close(result) {
      if (!overlay.classList.contains("sna-confirm-visible")) return;
      _confirmDialogActive = false;
      overlay.classList.remove("sna-confirm-visible");
    ...
    cancelBtn.onclick = (e) => { e.preventDefault(); e.stopPropagation(); close(false); };
    okBtn.onclick = (e) => { e.preventDefault(); e.stopPropagation(); close(true); };
    document.addEventListener("keydown", blockKeys, true);

    overlay.querySelector(".sna-confirm-message").textContent = message;
    overlay.classList.add("sna-confirm-visible");
  });
}
window.showConfirmDialog = showConfirmDialog;

// React SPA: expose init so Home page can call it on mount (scripts loaded via index.html or dynamically)
function _homeInit() {
  initializeApp();
  navigateToPractice();
}
window.initHome = _homeInit;
...
function initializeApp() {
  ...
  // Refresh progress when returning to this page (e.g. back button, tab switch) - only once for SPA
  if (!window._homeGlobalListenersAdded) {
    window._homeGlobalListenersAdded = true;
...
function initializeMyTestsButtons() {
  ...
  if (purchaseTestBtn) {
    purchaseTestBtn.addEventListener("click", function () {
      if (window.__REACT_NAVIGATE) window.__REACT_NAVIGATE("index.html");
...
function addEventListeners() {
  ...
  // Close sidebar when clicking outside - only once for SPA
  if (!window._homeDocumentClickAdded) {
    window._homeDocumentClickAdded = true;
    document.addEventListener("click", function (event) {
      const sidebar = document.getElementById("sidebar");
      const menuBtn = document.querySelector(".menu-btn");
      if (sidebar && menuBtn && sidebarOpen &&
    ...
    if (skill) {
      card.addEventListener("click", function () {
        ...
        setTimeout(() => {
          ...
          if (url) {
            if (window.__REACT_NAVIGATE) window.__REACT_NAVIGATE(url);
...
function startFullPracticeTest() {
  ...
  setTimeout(() => {
    if (window.__REACT_NAVIGATE) window.__REACT_NAVIGATE("read and select.html");
    else window.location.href = "read and select.html";
  }, 500);
}
...
// Close menu when clicking outside - only once for SPA
if (!window._helpMenuDocumentClickAdded) {
  window._helpMenuDocumentClickAdded = true;
  document.addEventListener("click", function (event) {
...
// Update keyboard shortcuts to include help menu - only once for SPA
