...
// Quiz Data (will be loaded from JSON)
      ...
      async function loadQuizData() {
        try {
          ...
          const response = await fetch(getQuizDataUrl("speak about the photo data.json"));
          if (!response.ok) {
      ...
      function showQuestion() {
        ...
        // Set photo
        getEl(containerEl, "photoImage").src = currentQuestion.imageUrl;
      ...
      async function startRecording() {
        ...
        getEl(containerEl, "continueBtn").style.display = "block";
      ...
      const resultsHelper = initResultsScreen(containerEl, {
        skillName: "Speak About the Photo",
      ...
      const recordBtnEl = getEl(containerEl, "recordButton");
      if (recordBtnEl) { recordBtnEl.addEventListener("click", handleRecord); unbind.push(() => recordBtnEl.removeEventListener("click", handleRecord)); }
      const sampleBtnEl = getEl(containerEl, "sampleBtn");
      if (sampleBtnEl) { sampleBtnEl.addEventListener("click", toggleSample); unbind.push(() => sampleBtnEl.removeEventListener("click", toggleSample)); }
      const recordingBtnEl = getEl(containerEl, "recordingBtn");
      if (recordingBtnEl) { recordingBtnEl.addEventListener("click", toggleRecording); unbind.push(() => recordingBtnEl.removeEventListener("click", toggleRecording)); }
      loadQuizData();
