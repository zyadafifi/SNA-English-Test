...
// Quiz Data (will be loaded from JSON)
      ...
      async function loadQuizData() {
        try {
          ...
          const response = await fetch(getQuizDataUrl("interactive listening data.json"));
          if (!response.ok) {
      ...
      function generateWaveform() {
        const waveform = getEl(containerEl, "audioWaveform");
        if (!waveform) return;
        waveform.innerHTML = "";
      ...
      function showQuestion() {
        ...
        const questionsSection = getEl(containerEl, "questionsSection");
        ...
        if (playButton) playButton.className = "play-button";
        if (audioStatus) audioStatus.style.display = "none";
        if (questionsSection) populateQuestions(currentQuestionData.questions);
      ...
      function playAudio() {
        const playButton = getEl(containerEl, "playButton");
        const audioStatus = getEl(containerEl, "audioStatus");
        const waveformBars = containerEl.querySelectorAll(".waveform-bar");
        ...
        try {
          ...
          utterance.onstart = function () {
            ...
            waveformBars.forEach((bar, index) => {
              setTimeout(() => bar.classList.add("playing-animation"), index * 50);
            });
          };

          utterance.onend = function () {
            if (playButton) playButton.classList.remove("playing");
            waveformBars.forEach((bar) => bar.classList.remove("playing-animation"));
            hideAudioStatus();
          };
      ...
      function checkAllInputsFilled() {
        const inputs = containerEl.querySelectorAll(".question-input");
        const submitBtn = getEl(containerEl, "submitBtn");
        if (!submitBtn) return;
        ...
        if (allFilled) submitBtn.className = "submit-btn active";
        else submitBtn.className = "submit-btn";
      }

      function startQuestionTimer() {
        timerDbg.clearOrphanedInterval("InteractiveListening");
        if (timerInterval) {
        ...
        timerInterval = setInterval(() => {
          ...
          if (seconds >= 90 && timerElement) timerElement.classList.add("warning");
        }, 1000);
        timerDbg.registerInterval("InteractiveListening", timerInterval);
      }
      ...
      function stopQuestionTimer() {
        if (timerInterval) {
          timerDbg.unregisterInterval("InteractiveListening");
          clearInterval(timerInterval);
          timerInterval = null;
        }
        if (timeoutInterval) {
      ...
      function submitAnswers(isTimeout = false) {
        ...
        if (!isTimeout && !submitBtn.classList.contains("active")) return;
      ...
      function showFeedback(isCorrect, userAnswers, isTimeout) {
        ...
        const feedbackDetails = getEl(containerEl, "feedbackDetails");

        feedbackSection.style.display = "block";

        if (isCorrect) {
          playSound("right answer SFX.wav");
          if (feedbackSection) feedbackSection.className = "feedback-section correct";
          if (feedbackTitle) {
          ...
          if (feedbackSection) feedbackSection.className = "feedback-section incorrect";
          if (feedbackTitle) {
          ...
          if (feedbackDetails) feedbackDetails.innerHTML = feedbackHTML;
        }
      ...
      const resultsHelper = initResultsScreen(containerEl, {
        skillName: "Interactive Listening",
      ...
      const keydownHandler = function (event) {
        ...
        if (event.key === "Escape") {
          ...
          const submitBtn = getEl(containerEl, "submitBtn");
          if (submitBtn && submitBtn.classList.contains("active")) submitAnswers();
      ...
      const playBtn = getEl(containerEl, "playButton");
      if (playBtn) { playBtn.addEventListener("click", playAudio); unbind.push(() => playBtn.removeEventListener("click", playAudio)); }
      const submitBtnEl = getEl(containerEl, "submitBtn");
      const onSubmitClick = () => submitAnswers(false);
      if (submitBtnEl) { submitBtnEl.addEventListener("click", onSubmitClick); unbind.push(() => submitBtnEl.removeEventListener("click", onSubmitClick)); }
      const contBtn = getEl(containerEl, "continueBtn");
      if (contBtn) { contBtn.addEventListener("click", nextQuestion); unbind.push(() => contBtn.removeEventListener("click", nextQuestion)); }
      loadQuizData();
