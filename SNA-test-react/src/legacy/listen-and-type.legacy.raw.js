// Quiz Data (will be loaded from JSON)
      ...
      if (!document.getElementById("waveform-animation-style")) {
      ...
      function showResults() {
        ...
        if (continueBtn) {
          ...
          const hasNextQuiz = typeof getNextQuizUrl === "function" && getNextQuizUrl("Listen and Type");
      ...
      function continueToNextStage() {
        if (stagesCompletedThisSession >= STAGES_TOTAL && typeof getNextQuizUrl === "function") {
          const nextUrl = getNextQuizUrl("Listen and Type");
          if (nextUrl) {
      ...
      function calculateListeningAccuracy() {
        ...
        return validAnswers > 0
          ? Math.round(totalSimilarity / validAnswers)
          : 0;
      }
      ...
      function calculateSessionScore(sessionData) {
        ...
        const typingSpeedScore = Math.max(
          0,
          100 - (speedDiff / optimalSpeed) * 50
        );
      ...
      function updateSkillOverallAssessment(skillName, allSessions) {
        ...
        const avgListeningAccuracy =
          allSessions.reduce(
            (sum, session) => sum + session.listeningAccuracy,
            0
          ) / allSessions.length;
      ...
      function generateRecommendations(sessions, finalScore) {
        ...
        if (commonErrors.mostCommon === "spelling") {
          ...
          recommendations.push(
            "Listen more carefully to avoid adding extra words"
          );
        }
