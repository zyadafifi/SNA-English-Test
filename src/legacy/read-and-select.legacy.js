...
// Quiz Data (will be loaded from JSON)
      ...
      async function loadQuizData() {
        try {
          ...
          const response = await fetch(getQuizDataUrl("read and select data.json"));
          if (!response.ok) {
      ...
      function showQuestion() {
        ...
        getEl(containerEl, "wordDisplay").textContent = question.word;

        // Reset buttons
        const yesBtn = getEl(containerEl, "yesBtn");
        const noBtn = getEl(containerEl, "noBtn");
      ...
      // Start Question Timer (matches reference: read and select.html)
      function startQuestionTimer() {
        timerDbg.clearOrphanedInterval("ReadAndSelect");
        if (timerInterval) {
        ...
        timerDbg.registerInterval("ReadAndSelect", timerInterval);
      }

      // Stop Question Timer
      function stopQuestionTimer() {
        if (timerInterval) {
          timerDbg.unregisterInterval("ReadAndSelect");
          clearInterval(timerInterval);
          timerInterval = null;
        }
      ...
      function playSound(filename) {
        try {
          const audio = new Audio(filename.startsWith("/") ? filename : "/" + filename);
      ...
      const resultsHelper = initResultsScreen(containerEl, {
        ...
        selectors: { resultsScreen: "resultsScreen", resultsScore: "resultsScore", resultsContinueBtn: "resultsContinueBtn", doneBtn: ".done-btn", questionCard: "questionCard", questionCardDisplay: "flex", hideWhenResults: ["feedbackSection"] },
      });
      ...
      const yesBtnEl = getEl(containerEl, "yesBtn");
      if (yesBtnEl) { const h = () => selectAnswer(true); yesBtnEl.addEventListener("click", h); unbind.push(() => yesBtnEl.removeEventListener("click", h)); }
      const noBtnEl = getEl(containerEl, "noBtn");
      if (noBtnEl) { const h = () => selectAnswer(false); noBtnEl.addEventListener("click", h); unbind.push(() => noBtnEl.removeEventListener("click", h)); }
      loadQuizData();
