...
// Quiz Data (will be loaded from JSON)
      ...
      async function loadQuizData() {
        try {
          ...
          const response = await fetch(getQuizDataUrl("write about photo data.json"));
          if (!response.ok) {
      ...
      function showQuestion() {
        ...
        // Set photo
        getEl(containerEl, "photoImage").src = question.imageUrl;

        // Clear textarea
        const textarea = getEl(containerEl, "responseTextarea");
        textarea.value = "";
      ...
      function updateCharCounter() {
        const textarea = getEl(containerEl, "responseTextarea");
        const charCounter = getEl(containerEl, "charCounter");
      ...
      function startQuestionTimer() {
        timerDbg.clearOrphanedInterval("WriteAboutPhoto");
        if (timerInterval) {
        ...
        timerInterval = setInterval(() => {
          ...
          if (seconds >= 50 && timerElement) timerElement.classList.add("warning");
        }, 1000);
        timerDbg.registerInterval("WriteAboutPhoto", timerInterval);
      }
      ...
      function stopTimers() {
        if (timerInterval) {
          timerDbg.unregisterInterval("WriteAboutPhoto");
          clearInterval(timerInterval);
          timerInterval = null;
        }
        if (writingTimer) {
      ...
      function showFeedback(sampleAnswer) {
        ...
        const sampleAnswerText = getEl(containerEl, "sampleAnswerText");
      ...
      const resultsHelper = initResultsScreen(containerEl, {
        skillName: "Write About the Photo",
      ...
      if (contBtn) { contBtn.addEventListener("click", submitAnswer); unbind.push(() => contBtn.removeEventListener("click", submitAnswer)); }
      loadQuizData();
