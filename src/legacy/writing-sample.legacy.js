...
// Quiz Data
      ...
      async function loadQuizData() {
        try {
          ...
          const response = await fetch(getQuizDataUrl("writing sample data.json"));
          if (!response.ok) {
      ...
      function setupPreparationStage() {
        ...
        getEl(containerEl, "preparationTitle").textContent =
          quizData.preparation.title;
        getEl(containerEl, "preparationSubtitle").textContent =
          quizData.preparation.subtitle;
        getEl(containerEl, "topicText").textContent = quizData.topic;
      ...
      function startWriting() {
        ...
        getEl(containerEl, "preparationStage").style.display = "none";

        // Show writing stage
        const writingStage = getEl(containerEl, "writingStage");
        writingStage.style.display = "block";

        // Populate writing content
        getEl(containerEl, "writingTitle").textContent =
          quizData.writing.title;
        getEl(containerEl, "writingTopic").textContent = quizData.topic;

        // Update timer label

        // Focus on textarea
        const writingArea = getEl(containerEl, "writingArea");
        writingArea.focus();
      ...
      function checkWritingInput() {
        const writingArea = getEl(containerEl, "writingArea");
        const continueBtn = getEl(containerEl, "writingContinue");
      ...
      const resultsHelper = initResultsScreen(containerEl, {
        ...
        onSelectNextStage() {
          ...
          getEl(containerEl, "writingStage").style.display = "none";
        ...
        selectors: { resultsScreen: "resultsStage", resultsScore: "resultsScore", resultsContinueBtn: "resultsContinueBtn", doneBtn: ".done-btn", questionCard: "preparationStage", questionCardDisplay: "block", hideWhenResults: ["writingStage"] },
      });
      ...
      function showResults() {
        ...
        getEl(containerEl, "timer").style.display = "none";
        getEl(containerEl, "resultsTitle").textContent = quizData.results.title;
        getEl(containerEl, "resultsTopic").textContent = quizData.topic;
        getEl(containerEl, "userResponse").textContent = userResponse;
        getEl(containerEl, "sampleTitle").textContent = quizData.results.sampleTitle;
        getEl(containerEl, "sampleText").textContent = quizData.results.sampleAnswer;
        resultsHelper.showResults({ correct: 1, total: 1, stagesCompletedThisSession });
      }

      // Timer Functions (matches reference: writing sample.html)
      ...
      function startWritingTimer() {
        ...
        timerInterval = setInterval(() => {
          ...
          const remainingSeconds = seconds % 60;
          if (timerText) timerText.textContent = `${minutes}:${remainingSeconds.toString().padStart(2, "0")}`;
          if (seconds >= 240 && timerElement) timerElement.classList.add("warning");
        }, 1000);
      ...
      if (errScreen) {
        const errorBtns = errScreen.querySelectorAll(".retry-btn, .retry-data-btn");
      ...
      const prepCont = getEl(containerEl, "preparationContinue");
      if (prepCont) { prepCont.addEventListener("click", startWriting); unbind.push(() => prepCont.removeEventListener("click", startWriting)); }
      const writingCont = getEl(containerEl, "writingContinue");
      if (writingCont) { writingCont.addEventListener("click", showResults); unbind.push(() => writingCont.removeEventListener("click", showResults)); }
      loadQuizData();
